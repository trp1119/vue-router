...
export default [
  ...
  },
  {
    ...
    //path: '/login',
    path: '/login/:id', // 传参，类似 /login/xxx ，在页面中通过 this.$route.params 可以拿到键值对
    // props: true, // 定义 props 为 true 后，可以将 :id 作为 props 传入组件 Login 中，即在组件中，用 props 直接接收即可 props: ['id']，代替父组件传值。这种方式可以实现在组件内不需要写 this.$route 这种写法
